...
fprintf('Grid Search Average Biases SVD\n');
fprintf('Lambda_1,Mean_RMSE,Std_RMSE,Mean_CPU,Std_CPU\n');
for lam1=1:30
    for k=1:nFolds
      idx_trn = idxs_trn(k,:);
      idx_tst = idxs_tst(k,:);
      ...
      tic;
      [X_pred] = Baseline_SVD(X_trn, lam1, X_tst, nil);
      times(k) = toc;
    ...
    fprintf('%d,%f,%f,%f,%f\n', ...
            lam1, mean(rmses), std(rmses), mean(times), std(times));
end
